...
[parameter(Mandatory=$false)]  [ValidateSet('ALL', 'ODBCORACLE', 'MYSQL')] [string] $DBTYPE = 'ALL'
)

Set-DefaultAWSRegion -Region 'us-east-1' -Scope Script

if ($DBTYPE -EQ 'MYSQL' -OR ($DBTYPE -EQ 'ALL') ) {
...
if ($DBTYPE -EQ 'ODBCORACLE' -OR ($DBTYPE -EQ 'ALL') ) {
